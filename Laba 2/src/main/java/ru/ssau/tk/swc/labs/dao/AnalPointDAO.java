...
public class AnalPointDAO {
    private static final Logger logger = LoggerFactory.getLogger(AnalPointDAO.class);
    private final DataSourceProvider dataSourceProvider;

    public AnalPointDAO(DataSourceProvider dataSourceProvider) {
        this.dataSourceProvider = dataSourceProvider;
    }

    public Optional<AnalPoint> findByID(Long id){
        String sql = "SELECT * FROM anal_points WHERE id = ?";
        ...
        try(Connection conn = dataSourceProvider.getConnection();
            ...
            try(ResultSet rs = stmt.executeQuery()) {
                if(rs.next()){
                    AnalPoint analPoint = mapResultSetToAnalPoint(rs);
                    logger.info("Точка под id: {} найдена", id);
                    return Optional.of(analPoint);
                }
            }
    ...
    public List<AnalPoint> findAll() {
        String sql = "SELECT * FROM anal_points";
    ...
    public Long create(AnalPoint point) {
        String sql = "INSERT INTO anal_points (x, y, derive, funID) VALUES (?, ?, ?, ?)";
    ...
    public boolean delete(Long id) {
        String sql = "DELETE FROM anal_points WHERE id = ?";
    ...
    private AnalPoint mapResultSetToAnalPoint(ResultSet rs) throws SQLException {
        ...
        analPoint.setX(rs.getDouble("x"));
        analPoint.setY(rs.getDouble("y"));
        analPoint.setDerive(rs.getDouble("derive"));
        analPoint.setFunID(rs.getLong("funID"));
        return analPoint;
    }
}